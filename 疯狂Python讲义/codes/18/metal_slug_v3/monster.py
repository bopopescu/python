...
class Monster(Sprite):
    ...
    def draw_anim(self, screen, view_manager, bitmap_arr):
        # 如果怪物已经死，且没有播放过死亡动画
        # （self.die_max_draw_count 等于初始值表明未播放过死亡动画）
        if self.is_die and self.die_max_draw_count == sys.maxsize:
            # 将die_max_draw_count 设置与死亡动画的总帧数相等
        ...
        if self.draw_count >= (COMMON_SPEED_THRESHOLD if type == TYPE_MAN
            ...
            self.draw_index += 1  # ②
            self.draw_count = 0  # ④
        # 每播放死亡动画的一帧，self.die_max_draw_count 减 1。
        # 当self.die_max_draw_count 等于 0 时，表明死亡动画播放完成。
        if self.is_die:
    ...
    def add_bullet(self):
        ...
        # 将子弹添加到该怪物发射的子弹 Group 中
        self.bullet_list.add(bullet)

    # 更新所有子弹的位置：将所有子弹的X坐标减少 shift 距离（子弹左移）
