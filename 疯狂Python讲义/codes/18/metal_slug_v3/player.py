...
class Player(Sprite):
    def __init__(self, view_manager, name, hp):
        ...
        # 控制射击状态的保留计数器
        # 每当用户发射一枪时，left_shoot_time 会被设为 MAX_LEFT_SHOOT_TIME，然后递减
        # 只有当 left_shoot_time 变为 0 时，用户才能发射下一枪
    ...
    def draw_anim(self, screen, leg_arr, head_arr_from, pdir):
        ...
        # self.draw_count 控制该方法每调用 4 次才会切换到下一帧位图
        self.draw_count += 1
    ...
    def add_bullet(self, view_manager):
        ...
        # 将子弹添加到用户发射的子弹 Group 中
        self.bullet_list.add(bullet)
        # 发射子弹时，将 self.left_shoot_time 设置为射击状态最大值
